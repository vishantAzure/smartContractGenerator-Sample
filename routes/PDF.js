...
// var MilestoneDB = require('../modals/milestone.js');
...
router.post('/UpdateTemplate', function(req, res) {
    var sql = 'UPDATE Templates SET TemplateName = ?, Category_Name = ?, Status = ?, Description = ? WHERE idTemplate = ?';
...
router.post('/DeleteTemplate', function(req, res) {
    ...
    var values = [[req.body.id,'InActive'],];
        ...
        if(result1.length>0) {
            global.con.query(sql,[values],function(err2,result2) {
                ...
                if(result2.length>0) {
                    ...
                    return res.json({status:200,res:{string1:str,string2:str2}});
                ...
                if(result2.length>0) {
                    ...
                    GeneratePdf(str , str2).then(function(response) {
                        ...
                        global.con.query(sql,[values],function(err5,result5) {
                            if(err5) return res.json({status:500,res:err5});
                            ...
                            global.con.query(sql,[values],function(err3,result3) {
                                if(err3) return res.json({status:500,res:err3});
    
                                if(result3.affectedRows==1) {
    
                                    for(var i=0;i<req.body.milestone.length;i++) {
    
                                        var sql = "INSERT INTO Milestones (idMilestone,Start_Date, End_Date, Contract_ID, Task_Status, Payment_Status, Contract_Name, Customer_Name, Milestone_fee, Milestone_schedule, Milestone_Work_execution) VALUES ?";
                                        var values = [[req.body.milestone[i].ID, req.body.milestone[i].StartDate, req.body.milestone[i].EndDate, response, 'Pending', 'Pending', req.body.ID.ContractName, 'Customer', req.body.milestone[i].Fees, req.body.milestone[i].Schedule, req.body.milestone[i].WorkExecution],];
                                        
                                        global.con.query(sql,[values],function(err4,result4) {
                                            if(err4) return res.json({status:500,res:err4});
...
//     .then(() => MilestoneDB.create({
//         idMilestone: 1,
//         Start_Date: new Date(),
//         End_Date: new Date(),
//         Contract_ID :'fjvnijnvjdfnvjndfvdf444444ddfvdf',
//         Task_Status: 0,
//         Payment_Status :0,
//         Contract_Name :'ffffffff',
//         Customer_Name :'vvvvvvvv',
//         Milestone_fee :5555555,
//         Milestone_schedule :'ccccccccccccccccccccccvvvvvvvvvvvvvvvvvvvvvbbbbbbbbbbbb',
//         Milestone_Work_execution: 'mfjfjffjfjjffjfjjjfjjfjffjjffjjfjfjfjffjjjjffff'
//     }))
//     .then(res => {
//       console.log(res.get({
//         plain: true
